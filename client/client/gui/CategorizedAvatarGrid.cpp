...
namespace card {
	CategorizedAvatarGrid::CategorizedAvatarGrid(AvatarTextures& avatarTextures, Avatar chosenAvatar) :
		...
		maleLabel->setYTranslation(1 * PADDING_PX);
		...
		maleGrid->setYTranslation(1 * PADDING_PX);
		...
		femaleLabel->setYTranslation(2 * PADDING_PX);
		...
		femaleGrid->setYTranslation(2 * PADDING_PX);
