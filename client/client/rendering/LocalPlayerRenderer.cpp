...
namespace card {
	...
	void LocalPlayerRenderer::renderHandCards(ProxyMauMauGame& game) {
		std::optional<int> intersectedCardOrNone = checkIntersectionWithHandCards(game);
		...
		if(intersectedCard != intersectedCardIndexInLastFrame) {
			intersectedCardIndexInLastFrame = intersectedCard;
			unixTimeIntersectedCardIndexHasChanged = getMilliseconds();
		}
		...
		handCardStackRenderer.renderCardStackInX(localPlayer->getCardStack(), HAND_CARDS_LOCAL_POSITION, HAND_CARDS_LOCAL_ROTATION, projectionMatrix, viewport, FRONT_BACK_OPPONENT_CARDS_WIDTH, intersectedCard, addition);
	}
