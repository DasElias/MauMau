...
namespace card {
	MessageRenderer::TextToRender::TextToRender(int fontSizePx) {
		this->text = std::make_shared<egui::Label>("", fontSizePx, false, egui::Text::HorizontalAlignment::CENTER, egui::Text::VerticalAlignment::TOP, egui::Color(1, 1, 1), egui::Font::getFont("Roboto Condensed"));
		this->shadow = std::make_shared<egui::Label>("", fontSizePx, false, egui::Text::HorizontalAlignment::CENTER, egui::Text::VerticalAlignment::TOP, egui::Color(0, 0, 0), egui::Font::getFont("Roboto Condensed"));
		shadow->setXTranslation(7);
		shadow->setYTranslation(7);
		addChildElement(shadow);
		addChildElement(text);

		this->positioning = std::make_shared<egui::RelativePositioningOnScreen>(0.0f, 0.0f);
	...
	void MessageRenderer::TextToRender::setY(float yRelativeOnScreen) {
		positioning->setY(yRelativeOnScreen);
	}
	MessageRenderer::MessageRenderer(egui::MasterRenderer& eguiRenderer) :
			eguiRenderer(eguiRenderer) {

		int const FONT_SIZE_PX = 65;
		auto parent = std::make_shared<egui::UnorganizedParentElement>();
